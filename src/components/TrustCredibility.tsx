...
export default function TrustCredibility() {
  return (
    <section id="statistics" className="py-[100px] md:py-[140px] lg:py-[180px] bg-gradient-to-b from-white via-[#F0F9FF] to-white relative overflow-hidden">
      ...
      <div className="absolute bottom-0 right-0 w-[450px] h-[450px] bg-[#3D8DC3]/5 rounded-full blur-[130px] pointer-events-none"></div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 max-w-5xl mx-auto">
          {TRUST_STATS.map((stat, idx) => (
            <motion.div
              ...
              key={idx}
              className="bg-white border border-[#E0F2FE] hover:border-[#1C8DC8]/40 rounded-2xl sm:rounded-[28px] p-3 xs:p-4 sm:p-8 flex flex-col items-center justify-between text-center shadow-[0_12px_24px_rgba(28,141,200,0.02)] hover:shadow-[0_16px_36px_rgba(28,141,200,0.08)] hover:bg-[#F0F9FF]/20 transition-all duration-300 group"
            >
              <div className="space-y-1.5 sm:space-y-3">
                <span className="font-display font-[900] text-xl xs:text-2xl sm:text-4xl lg:text-5xl text-transparent bg-clip-text bg-gradient-to-br from-[#1C8DC8] via-[#146299] to-[#0B3951] block tracking-tighter">
                  <AnimatedCounter value={stat.value} />
                </span>
                <h4 className="font-display font-extrabold text-[10px] xs:text-xs sm:text-sm text-[#0B3951] tracking-tight leading-snug">
                  {stat.label}
                </h4>
              </div>
