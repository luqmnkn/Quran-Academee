...
import { ArrowRight, BookOpen, Clock, X, CheckCircle2, Mic } from 'lucide-react';
...
export default function Services({ onSelectCourse }: ServicesProps) {
  ...
  // Lock scroll & bind Escape key when any modal is active
  useEffect(() => {
    const isModalOpen = activeCourse !== null || isRecorderOpen;
  ...
  }, [activeCourse, isRecorderOpen, closeModal]);
  ...
  const renderAssessmentCard = () => (
    <div className="bg-gradient-to-br from-[#0B3951] via-[#0E4967] to-[#146299] text-white rounded-2xl border border-[#1C8DC8]/50 p-5 sm:p-6 flex flex-col justify-between h-full transition-all duration-300 hover:shadow-[0_15px_35px_rgba(28,141,200,0.18)] hover:border-[#1C8DC8]/80 hover:-translate-y-1.5 group relative overflow-hidden text-left min-h-[330px]">
      {/* Accent Indicator */}
      <div className="absolute top-0 left-0 right-0 h-[4px] bg-gradient-to-r from-[#1C8DC8] to-[#3D8DC3] scale-x-0 group-hover:scale-x-100 transition-transform duration-300" />
      <div className="absolute -top-12 -right-12 w-28 h-28 bg-[#1C8DC8]/10 rounded-full blur-2xl group-hover:bg-[#1C8DC8]/20 transition-all duration-500 pointer-events-none" />
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center space-x-1.5">
            <h3 className="font-display font-[900] text-lg text-white tracking-tight leading-snug">
              Recitation Level Assessment
            </h3>
            <span className="bg-[#1C8DC8] text-white text-[8px] font-mono font-black uppercase tracking-wider px-1.5 py-0.5 rounded">
              FREE
            </span>
          </div>
  ...
  const getGoogleIcon = (iconName: string) => {
    ...
    return GOOGLE_ICON_MAP[normalized] || 'menu_book';
  };
          ...
          >
            ...
            >
              <div className="bg-[#0B3951] text-white p-8 relative overflow-hidden text-left">
                <div className="absolute right-0 top-0 w-44 h-44 bg-[#1C8DC8]/10 rounded-full blur-2xl" />
